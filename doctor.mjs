...
function checkCv() {
  ...
  return {
    ...
    fix: 'Create cv.md in the project root with your CV in markdown',
  };
}
...
function checkPortals() {
  const configPortals = join(projectRoot, 'config', 'portals.yml');
  if (existsSync(configPortals)) {
...
async function main() {
  ...
  if (failures > 0) {
    ...
    console.log('Result: All checks passed. See jobhunt/RUNBOOK.md for the pipeline.');
    process.exit(0);
  }
}
