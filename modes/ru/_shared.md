...
Оценка использует 6 блоков (A-F) с **глобальным баллом по шкале 0-10**:
...
| Компенсация | Зарплата vs рынок (10 = верхний квартиль, 0 = значительно ниже) |
...
- 9.0+ → Сильное совпадение, рекомендуется откликнуться сразу
- 8.0-8.9 → Хорошее совпадение, стоит откликнуться
- 7.0-7.9 → Нормально, но не идеально; откликаться только при наличии конкретной причины
- Ниже 7.0 → Настоятельно рекомендуется не откликаться

> **Канонический порог (Ethical Use):** CLAUDE.md устанавливает порог **8.0/10** — при баллах ниже 8.0 агент настоятельно рекомендует не откликаться. Диапазон 7.0–7.9 допускается только при наличии весомой причины и требует явного решения пользователя.
